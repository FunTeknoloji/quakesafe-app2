...
class OtherScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Hizmetler", style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.black,
        elevation: 0,
      ),
    ...
    return GestureDetector(
      onTap: () {
        if (item.screen != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("${item.title} yakında eklenecek.")));
        }
      },
      child: Container(
  ...
  static final List<OtherItem> _items = [
    OtherItem("Sesli Komut", "ELLER SERBEST KONTROL", Icons.mic, Colors.blue, screen: const VoiceControlScreen()),
    OtherItem("Haberler", "SON DAKİKA GELİŞMELERİ", Icons.newspaper, Colors.orange, screen: const NewsScreen()),
    ...
    OtherItem("Afet Planı", "AİLE HAZIRLIK REHBERİ", Icons.assignment, Colors.red, screen: const DisasterPlanScreen()),
    OtherItem("Sağlık Haritası", "EN YAKIN HASTANELER", Icons.local_hospital, Colors.pink),
    OtherItem("Hava Durumu", "ANLIK METEOROLOJİ", Icons.cloud, Colors.lightBlue, screen: const WeatherScreen()),
    ...
    OtherItem("Acil Çağrı", "TEK TUŞLA YARDIM", Icons.phone_in_talk, Colors.redAccent, screen: const EmergencyCallScreen()),
  ];
}
